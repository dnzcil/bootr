...
# boot function
boot_function = function(x, i, model, formula, formula_match, cem_cutpoints, treatment, predictions, predictions_func, cpanel, unscale = NULL, ...) {
  # data
  dt = x[i,]
...
bootr = function(numruns = 1000, model = lm, formula, formula_match = NULL, cem_cutpoints = NULL, data, predictions = NULL, omit = NULL) {
  ...
  bout = boot(data, boot_function, numruns, parallel = "snow", ncpus = numcores, #sim = "ordinary", stype = "i", strata = rep(1, nrow(data)),
           ...
           predictions = predictions, predictions_func = bootr.predictions, cpanel = cpanel)
